...
Tree::Tree(int depth) : Placeable() {
	this->depth = depth;
	this->root = generate(new RoseTree(.08, 1.0, 0, 0, depth), depth, depth);

	this->barkTexture = TextureManager::Get()->Load(TEX_BARK);
	this->leafTexture = TextureManager::Get()->Load(TEX_LEAF);

	Placeable::SetScale(0.15);
...
int Tree::GetRowFootPrint() {
	return 3;
}

int Tree::GetColumnFootPrint() {
	return 3;
}
...
void Tree::drawCanopy() {
	glDisable(GL_CULL_FACE);
	glBindTexture(GL_TEXTURE_2D, leafTexture);

	float r = .3;			// Radius of canopy ball
	float W = 0.25 * 1.0;	// Width of leaf
	float L = 0.25 * 0.2;	// Length of leaf
...
void Tree::drawTree(RoseTree* roseTree) {
...
RoseTree* Tree::generate(RoseTree* parent, int numChildren, int depth) {
	if (depth == 0 || numChildren == 0)
		return parent;

	int sign = 1;
	for (int i = 0; i < numChildren; i++, sign *= -1)
		parent->Children.push_back(generate(
		new RoseTree(
		0.5 * parent->GetBaseRadius(),
		0.5 * parent->GetLength(),
		360.0 * i / (numChildren),
		Random::Get()->NextInt(66),
		depth
		),
		numChildren - 1,
		depth - 1
...
void Tree::texturedQuad(float width, float height, int texture) {
	float segments = 7;
