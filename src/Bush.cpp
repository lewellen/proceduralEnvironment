...
Bush::Bush(int depth) : Placeable() {
	this->depth = depth;
	this->root = generate(new RoseTree(.1, 1, 0, 0, depth), depth, depth);
...
int Bush::GetRowFootPrint() {
	return 3;
}

int Bush::GetColumnFootPrint() {
	return 3;
}
...
void Bush::drawTree(RoseTree* roseTree) {
...
RoseTree* Bush::generate(RoseTree* parent, int numChildren, int depth) {
	...
	for (int i = 0; i < numChildren; i++) {
		parent->Children.push_back(generate(
			new RoseTree(
			0.95 * parent->GetBaseRadius(),
			1.05 * parent->GetLength(),
			360.0 / numChildren * i,
			45,
			depth
			),
			numChildren - 1, depth - 1
			));
	}
...
void Bush::texturedQuad(float width, float height, int texture) {
	float segments = 7;
